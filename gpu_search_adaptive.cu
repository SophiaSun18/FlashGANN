...
void QuantizationGraph::gpu_search_adaptive(
    ...
    const size_t required_bytes = query_bytes + results_bytes + result_dists_bytes + qg_data_bytes + qg_signs_bytes;
    ...
    auto query_transfer_start = std::chrono::high_resolution_clock::now();
    CUDA_SAFE_CALL(cudaMemcpy(d_queries, queries, query_bytes, cudaMemcpyHostToDevice));
    CUDA_SAFE_CALL(cudaDeviceSynchronize());
    auto query_transfer_end = std::chrono::high_resolution_clock::now();
    elapsed[QG_TIMER_QUERY_TRANSFER] =
        std::chrono::duration<double>(query_transfer_end - query_transfer_start).count();
    ...
    kernel<<<num_blocks, num_threads, shm_size>>>(
        ...
        this->num_nodes_, d_queries, d_qg_data, d_qg_signs, d_qg_levels, d_results, d_result_dists,
    ...
    elapsed[QG_TIMER_SEARCH] = std::chrono::duration<double>(end - start).count();
    auto result_copy_start = std::chrono::high_resolution_clock::now();
    ...
    CUDA_SAFE_CALL(cudaDeviceSynchronize());
    auto result_copy_end = std::chrono::high_resolution_clock::now();
    elapsed[QG_TIMER_RESULT_COPY] =
